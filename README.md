...
Automated runner that queries OpenAI and Perplexity models 3–4 times daily with a fixed prompt set, then tracks cited domains and logs results.

## How it works
- Prompts and target domains are read from external files so they can change without code edits.
- Each prompt is sent in a fresh session to OpenAI (`gpt-4.1`) and Perplexity (`sonar-pro`) with deterministic parameters and JSON-enforced formatting.
